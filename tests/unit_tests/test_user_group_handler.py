...
def test_create_user_group(mock_db_find_one, mock_db_create, test_client):
    ...
    mock_db_create.return_value = UserGroup(
        name="runtime:pull-labs-demo:node-editor"
    )
...
def test_delete_user_group(
    mock_db_find_by_id, mock_db_count, mock_db_delete_by_id, test_client
):
...
def test_delete_user_group_when_assigned(
    mock_db_find_by_id, mock_db_count, test_client
):
