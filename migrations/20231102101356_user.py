...
def upgrade(db: "pymongo.database.Database"):
    ...
    for user in users:
        ...
        is_superuser = False
        new_groups_list = [
            g for g in user["profile"]["groups"] if g["name"] != "admin"
        ]
...
def downgrade(db: "pymongo.database.Database"):
    ...
    if superusers:
        ...
        db.usergroup.update_one(
            {"name": "admin"}, {"$setOnInsert": {"name": "admin"}}, upsert=True
        )
    ...
    for user in users:
        ...
        if user.get("is_superuser"):
            ...
            new_groups_list = [
                g for g in user["groups"] if g["name"] != "admin"
            ]
