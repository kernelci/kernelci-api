...
the application opens a connection to an STMP server, and send the report to an user or a list of
recipients that are waiting for the report with the build information after executing tests and
possible regressions. With the report it is possible to see the number of tests have failed or passed,
