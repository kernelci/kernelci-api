...
class UserManager(ObjectIDIDMixin, BaseUserManager[User, PydanticObjectId]):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._template_env = jinja2.Environment(loader=jinja2.PackageLoader("api", "templates"))
    ...
    async def on_after_forgot_password(self, user: User, token: str, request: Optional[Request] = None):
        ...
        content = template.render(username=user.username, token=token)
        self.email_sender.create_and_send_email(subject, content, user.email)
    ...
    async def on_after_reset_password(self, user: User, request: Optional[Request] = None):
        ...
        template = self._template_env.get_template("reset-password-successful.jinja2")
