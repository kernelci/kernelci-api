...
class PubSub:  # pylint: disable=too-many-instance-attributes
    ...
    async def _update_subscriber_state(
        self, subscriber_id: str, last_event_id: int, last_poll: datetime = None
    ):
    ...
    async def _get_missed_events(
        ...
        *,
        ...
        col = self._mongo_db[self.EVENT_HISTORY_COLLECTION]
        query = {"channel": channel, "sequence_id": {"$gt": after_seq_id}}
    ...
    async def _update_listen_subscription_state(
        ...
        if subscriber_id and sub_data.get("last_delivered_id"):
            await self._update_subscriber_state(
                subscriber_id, sub_data["last_delivered_id"], datetime.utcnow()
            )
    ...
    def _should_deliver_to_user(
        ...
        return not ("owner" in msg_data and msg_data["owner"] != sub.user)
